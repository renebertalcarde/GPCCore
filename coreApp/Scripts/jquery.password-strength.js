...
(function($) {
    ...
    var getXML = function (xmlUrl) {
        ...
        $.ajax({
            type: "GET",
            url: xmlUrl, //use absolute link if possible
            dataType: "xml",
    ...
    var defaultText = 'Empty password!';

    $.fn.password_strength_clear = function () {
        var jpsCont = $(this).siblings('#jpscont');
        jpsCont.find('span').text(defaultText);
        jpsCont.find('#PasswordStrengthBar').css({ width: '0%', 'background-color': password_settings.barColor });
    };
    ...
    $.fn.password_strength = function(options) {
        ...
        getXML(options.xmlUrl);

        return this.each(function() {
            
            //password indicator text container
            var container = $('<span></span>').text(defaultText);
            //.css({ 'font-size': '75%', display: 'inline-block', width: password_settings.barWidth + 40 });
            
            //bar border and indicator div
            var passIndi = $('<div id="PasswordStrengthBorder"><div id="PasswordStrengthBar" class="BarIndicator"></div></div>');
            var jpsCont = $(this).siblings('#jpscont');

            jpsCont.find('.jpsmeter')
                .append(passIndi)
                .append(container);
            ...
            $(this).keyup(function() {
                ...
                var psBar = $("[id='PasswordStrengthBar']");
                ...
                if (passwordVal.length > 0) {
                    ...
                    if (password_settings.useMultipleColors === "1") {
                        //first 33% is red
                        if (strengthPercent >= 0 && strengthPercent <= 33) {
                            backColor = "red";
                        }
                        //33% to 66% is blue
                        else if (strengthPercent >= .33 && strengthPercent <= 67) {
                            backColor = "blue";
                        }
                        else {
                    ...
                    psBar.css({ width: strengthPercent + '%', 'background-color': backColor });
                    ...
                    psBar.css({ width: '0%', 'background-color': password_settings.barColor });                    
                }
            });
        });
    };
