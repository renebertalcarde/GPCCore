var filemanager = {
        ...
        me.toolbar.createFolder.click(function () {
            modalInput('Create New Folder', 'Specify new folder name', function (ret, val) {
                if (ret) {
                    ...
                    $.post(vdUrl(me.url_CreateFolder), data, function (res) {
                        if (res.IsSuccessful) {
        ...
        me.toolbar.deleteFolder.click(function () {
            modalConfirm('Are you sure you want to delete this folder including all folders and files in it?', function (ret) {
                if (ret) {
                    ...
                    $.post(vdUrl(me.url_DeleteFolder), data, function (res) {
                        if (res.IsSuccessful) {
                            me.showMessage(res.Remarks, false);
                            me.show(res.Data);
                        } else {
        ...
        me.toolbar.renameFolder.click(function () {
            modalInput('Rename Folder', 'Specify new folder name', function (ret, val) {
                if (ret) {
                    ...
                    $.post(vdUrl(me.url_RenameFolder), data, function (res) {
                        if (res.IsSuccessful) {
                            me.showMessage(res.Remarks, false);
                            me.show(res.Data);
                        } else {
        ...
        me.toolbar.deleteFile.click(function () {
            modalConfirm('Are you sure you want to delete the selected file?', function (ret) {
                if (ret) {
                    ...
                    $.post(vdUrl(me.url_DeleteFile), data, function (res) {
                        if (res.IsSuccessful) {
                            me.showMessage(res.Remarks, false);
                            me.show(res.Data);
                        } else {
        ...
        me.toolbar.renameFile.click(function () {
            modalInput('Rename File', 'Specify new filename', function (ret, val) {
                if (ret) {
                    ...
                    $.post(vdUrl(me.url_RenameFile), data, function (res) {
                        if (res.IsSuccessful) {
                            me.showMessage(res.Remarks, false);
                            me.show(res.Data);
                        } else {
    ...
    moveToFolder: function (srcXPath, destXPath) {
        ...
        $.post(vdUrl(me.url_MoveToFolder), data, function (res) {
            if (res.IsSuccessful) {
                me.showMessage(res.Remarks, false);
                me.show(res.Data);
            } else {
    ...
    show: function (xpath) {
        ...
        loadingUI(me.fileContainer, true);

        me.saveFolderStatus();

        me.fileContainer.load(vdUrl(this.url_GetFiles), function () {
            me.setDragDropUI();
            me.setCollapseUI();
            ...
            loadingUI(me.fileContainer, false);
        });
    },
